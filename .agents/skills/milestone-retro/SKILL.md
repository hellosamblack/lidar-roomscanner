...
ROADMAP/Codex/`web-protocol`/both memories were synced in the feature commit. This retro's extractions
...
- **Behaviour-change flag:** unifying the web + desktop config means a fresh web install now inherits the
  ...
  (my first assertion assumed `depth`). Documented in ROADMAP/Codex/memory. General lesson: when two
...
+ the new messages, memories + ROADMAP/Codex synced in the feature commit). This retro's two extractions:
...
- **Status-currency correction — "Showcase" collapsed (owner clarification).** The web plan carried a stale
  ...
  (`ROADMAP.md` header + the Phase-1/3/4 deferred lines, `AGENTS.md` bullet, `web-panel-replacement` +
...
closure-held state), and status was synced in the feature commit (ROADMAP/Codex/`web-panel-replacement`
...
- **`docs/web-protocol.md` created** — THE extraction. The `/ws` *app* protocol (browser ↔ FastAPI,
  ...
  server-side, off-loop blocking work). Pointers wired into AGENTS.md's docs list + ROADMAP Web Phase 3.
...
- **`docs/headless-host-setup.md`** — 5-minute checklist: the four gaps, the
  ...
  panel added this session. Pointer wired into AGENTS.md's docs list.
...
- **ROADMAP.md / AGENTS.md trued up**: Phase 5 marked complete (no longer shelved). The architecture decision was updated to reflect USB CDC OR Ethernet UDP as the transport links.
...
- **ROADMAP.md / AGENTS.md trued up**: Phases 3.5 + 4 marked complete with status blocks; Ethernet
