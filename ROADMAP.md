...
- **SLAM-8 — Distributed GPU compute backend (LAN multi-GPU)** *(proposed)*. Offload heavy SLAM/splat workloads to a faster GPU on the LAN. Candidates: **(a) Detailed SLAM** — send frame batches to a remote RTX 4080-class worker running ICP integrate/raycast/extract on CUDA, stream results back for replay display; **(b) Live ephemeral SLAM** — for exploratory scans or low-latency feedback without the local 7 ms/frame budget; **(c) 3D Gaussian Splat training** — run multi-GPU parallel training (local host + remote worker) or full offload to amortize 15–30 min trainer across hardware inventory. **Infrastructure needs:** gRPC or `aiozmq` transport for pose/mesh/loss backhaul; local-first fallback for displays when remote unavailable; worker health/queue depth instrumentation so the UI can show whether offload is active and not stalled. **Gating questions before prototyping:** (1) Is the inter-machine latency (network roundtrip) acceptable for live display at 30 Hz? (2) Does moving frame-by-frame integration off the reader thread (to wait on remote RPC) starve local live point-cloud rendering? (3) On a plausible 40 Mbps home network, is mesh bytes/s under the 5 MB/s backhaul budget? Start with a feasibility spike: measure E2E latency of a single frame (capture → worker integrate → extract → transmit) and decide (a) vs (b) precedence.
