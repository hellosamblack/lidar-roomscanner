...
- **SLAM-3 — Relocalization** *(open, architectural)*. The ICP retry (BUG-036) survives a bad *frame*,
  not a bad *second*; there is no recovery from a real tracking-kill (see fixture DC-C).

### Sensors / IMU (`SENS-`)

- **SENS-1 — Apply the +7.76 ms quat-phase lead.** On the wire since BUG-031 (`quat_mid_ticks`),
  nothing consumes it; it is now the largest motion-error term. Changes both the SLAM prior (SLAM-1)
  and the UI orientation, so it needs a before/after on a moving capture (**DC-F**).
