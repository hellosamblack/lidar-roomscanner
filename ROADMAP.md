...
| **Specced (queued)** | [SLAM pose-graph backbone](docs/superpowers/specs/2026-08-19-slam-posegraph-backbone-design.md)<br>[SLAM relocalization](docs/superpowers/specs/2026-08-19-slam-relocalization-design.md)<br>[SLAM integration admission control](docs/superpowers/specs/2026-08-19-slam-integration-admission-design.md)<br>[SLAM mesh-extraction ceiling](docs/superpowers/specs/2026-08-19-slam-mesh-ceiling-design.md)<br>[SLAM native engine candidates](docs/superpowers/specs/2026-08-19-slam-native-engine-design.md)<br>[Distributed GPU compute](docs/superpowers/specs/2026-08-19-slam-distributed-gpu-design.md)<br>[Offline 3DGS program](docs/superpowers/specs/2026-08-19-offline-3dgs-program-design.md)<br>[Android capture app](docs/superpowers/specs/2026-08-19-android-capture-design.md) | The 2026-08-19 spec pass (#206) converted the `priority/later` research backlog into eight design specs, each with a decision, a benchmark plan, and a kill criterion: **pose-graph backbone** (#110 #115 #150 #151 — offline pose graph first, GTSAM, hybrid per-node store, gravity + graph-end conventions; acceptance is the pre-registered `slam_loop_closure_gate` paired CI on clean-transport DC-B re-records, still blocked on #60), **relocalization** (#112 #154 — the IR-BoW branch was **killed by measurement**: ORB matching on 54×42 reflectance at a 1 s / 83%-overlap baseline is indistinguishable from unrelated frames, so v1 is zero-velocity re-anchor + pose-proximity), **integration admission control** (#149 #152 #153 — measured: RTAB-Map's 0.05 m/0.05 rad gate would skip **85%** of tripod and **68%** of room-sweep TSDF integrations; confidence is *already* a hard gate (`min_confidence=20`, provenance dangling) so the A/B is rejection vs. the unused per-point-weight hook in `cloud.py`), **mesh ceiling** (#111 — re-bisect on newest Open3D first, then a subprocess bisector MCP tool, then chunked extract-and-stitch; nvblox only as escalation), **native engine** (#113 #114 #116 — measured on real frames: small_gicp is ~4× faster end-to-end at 8 threads (2.2 vs 9.4 ms) and agrees with Open3D 6dof to 1.9 mm median, but its wheel has no rotation-prior mode and ICP is only ~10 ms of the ~35 ms frame, so staged gates with the null capture as assassin; no cutover commitment), **distributed GPU** (#117 — splat-training offload first, boring rsync+SSH v1; live offload presumptively killed by BUG-061/052 lessons), **offline 3DGS program** (#132 #133 #134 — posed captures from RTAB-Map/ARCore as the shared substrate, depth-`lambda` A/B in the existing trainer before any framework, Splatfacto as a one-round benchmark arm, SuGaR gated on beating the Detailed-SLAM mesh in `splat_compare`), and the **Android capture app** (#135 — enumeration probe first; plan A/plan B measurement order preserved; killed if RTAB-Map exports already suffice). Spike scripts were throwaway; `small_gicp` 1.0.1 and `opencv-python-headless` remain installed in `host/.venv` as evaluation candidates. |
