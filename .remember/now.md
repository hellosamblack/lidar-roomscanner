# Current State
- **Tracker triaged + docs/skills drift fixed (2026-08-11, `a06cdde`, closes #164):** every open issue now carries `priority/now|next|later`; SLAM critical path is **#60 -> #138 -> #110**, and most of the SLAM cluster (#150/#151/#154/#115/#156/#157) is downstream of #110's go/no-go. Two skill mechanisms were dead and are now execution-verified: `session-start`'s conflict gate (nonexistent `status/in-progress` label) and `session-end`'s marker (hardcoded `.git/` would clobber a worktree pointer). Plans/specs register now matches the tree exactly (39/39).
