...
static memory_t allocate_aligned_memory(uint16_t size);
...
void vl53l9_app() {
    ...
    vl53l9_utils_get_i3c_resolution(p_profile->binning, &in_width, &in_height);
...
#if defined(STM32N657xx)
    memory_t in_raw_mem[2] = { allocate_aligned_memory(raw_buffer_size), allocate_aligned_memory(raw_buffer_size) };
#else
...
#if defined(STM32N657xx)
        /* invalidate cache to ensure data coherency with the DMA transfer */
        SCB_InvalidateDCache_by_Addr((uint32_t *)in_raw_mem[raw_mem_index].data, in_raw_mem[raw_mem_index].size);
#endif
...
static void print_frame(float *p_frame, size_t height, size_t width) {
    ...
    for (unsigned int y = 0; y < height; y += pixel_step) {
        for (unsigned int x = 0; x < width; x += pixel_step) {
            unsigned int pixel_index = (y * width + x);
            uint32_t value = (uint32_t)p_frame[pixel_index];

            unsigned int ascii_index = (value - min) * (sizeof(ASCII_CHARS) - 1) / (max - min);
...
static memory_t allocate_aligned_memory(uint16_t size) {
    memory_t memory;
    memory.size = size;
    /* 32-byte align (D-cache line size) so cache maintenance cannot affect adjacent allocations */
    memory.data = aligned_alloc(32, (size + 31) & ~31);
    if (memory.data == NULL) {
        handle_error();
    }
    return memory;
