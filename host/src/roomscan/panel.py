...
.. deprecated:: Web Phase 5 (2026-07-16)
    The browser instrument (`roomscan-web`) is now the primary, supported UI --
    it covers real-time, sensors, record/playback, SLAM, and save, and unlike
    this panel it runs on the GPU-less headless host. This Open3D desktop panel
    is kept as unsupported legacy only (it needs a local display + Open3D GUI
    stack). Prefer `roomscan-web`. The shared, GUI-free reader plumbing this
    module and the web server both use now lives in `reader.py`.
...
# all four corners.
# _WORLD_UP is defined in reader.py (shared with the web server) and imported
# above; the comment block just above documents why fwd=+Z/up=-Y is the choice.
...
# _FOLLOW_BACK_OFF_M / _FOLLOW_LOOK_AHEAD_M are defined in reader.py (shared with
# the web follow-camera math) and imported above.
_FOLLOW_SMOOTH = 0.12
...
def run(args, *, smoke_ticks: int = 0) -> int:
    ...
    # Web Phase 5: this desktop panel is deprecated in favour of `roomscan-web`.
    # Surface it once at launch (stderr, not the log bus, so it shows even before
    # the GUI is up). Suppress in smoke tests to keep their output clean.
    if smoke_ticks == 0:
        print("[deprecated] roomscan-panel is legacy; the supported UI is "
              "`roomscan-web` (browser instrument, headless-capable). "
              "See ROADMAP.md 'Web replacement of panel.py'.", file=sys.stderr)
