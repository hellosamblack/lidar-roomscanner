...
Protocol on the wire: `/ws-mesh` sends binary `TAG_MESH` frames plus exactly one JSON text frame out,
`{"type": "mesh_reset", "generation": <int>}` (see below), and accepts one JSON message in,
`{"type": "mesh_ack", "seq": <mesh_seq>}` — see the inbound table below.

**`mesh_reset` — the source-generation barrier (#101, 2026-08-12).** Because `/ws-mesh` is a
*separate* socket from `/ws`, a mesh already queued for the old source can arrive *after* `/ws` has
announced the new one — the ordering hole that let a live mesh render under a loaded capture. On
every generation change the broadcaster clears `state.latest_mesh`, then sends `mesh_reset` on
`/ws-mesh` **before** that tick's `_pump_mesh`, so nothing for the superseded generation can follow
it. Each flow's `in_flight`/`last_sent_obj` are cleared with it, so a flow mid-ack on the old mesh
does not sit blocked for `MESH_ACK_TIMEOUT_S` before the new generation's first mesh can reach it.
`ws.js` parses it as a text control frame and re-emits it locally; `slam.js` drops its cached
geometry, trajectory and scanner pose on it.
...
| `state` | `source`(live\|view\|splat), `display`(point_cloud\|preview\|slam\|detailed), `selected_capture`, `selected_splat`, `slam_available`, `stream_generation`(int), `stream_ready`(bool), `detailed`{exists,current,stale,paths,manifest?}, plus the existing color/IR/view/SLAM preferences | `_state_message` | **`stream_generation` / `stream_ready` are the client reset barrier (#101, 2026-08-12).** `stream_generation` is `SessionController.generation`, bumped on every genuine source switch (`switch_to_live`/`switch_to_replay`) but **not** on `seek`/`restart`/loop-wrap, which are timeline discontinuities on the same capture, not source changes. `stream_ready` is `source != "view"` OR `_view_ready(...)` — for View it is true only once the controller is actually replaying *and* the file it is replaying matches `selected_capture`, so it stays false while the capture browser is open and during the window mid-swap before `replay_path` catches up. While it is false the server emits **no** point-cloud, IR, sensor, metrics or SLAM message at all (the reader may still be the live device), and the client keeps the capture browser visible. `ws.js` fires a local `stream_reset` event when the generation changes or readiness drops true→false — **edge-triggered on purpose**, since `state` re-broadcasts on every unrelated setting change and firing on each would constantly wipe a live view. `mode` remains a compatibility alias only; clients must use `source` + `display`. Preview is View-only and shows the selected capture's orientation-only thumbnail. `selected_splat` is the slug of the Gaussian-splat reconstruction the Splat source renders (`null` until one is picked). Detailed cache state is capture-keyed and never implies regeneration. The message is sent first on connect and after every source/display change. |
...
| `transport` | `action`(pause\|resume\|speed\|loop\|restart\|seek), `value` | playback control; `seek`/`restart` run off-loop via `to_thread` → echo `session`. **`seek` and `restart` begin a new replay timeline** and reset ephemeral SLAM + `SensorState`; `pause`/`resume`/`speed`/`loop` do not — see "Timeline discontinuities" below | `web.py:986` |
...
- **Timeline discontinuities reset ephemeral state (BUG-091 / issue #102, 2026-08-11).**
  `load_capture`, `go_live`, a `set_source` swap, `transport {action: "seek"}`,
  `transport {action: "restart"}` and **replay loop wraparound** all begin a new replay timeline:
  the frames after them are not a continuation of the frames before them. Each one calls
  `SessionController._begin_new_timeline()`, which drops the whole ephemeral pipeline —
  `SensorState.reset_source()` (latest quat, latest ENV/mag, pressure+temp histories and their
  sparklines, the stream-11 raw-IMU batch/history, the stream-12 tick trim back to nominal, and both
  fusion filters) followed by the `on_discontinuity` hook, wired in `create_app` to
  `reset_slam_state` (SLAM worker + TSDF + trajectory + the cached `/ws-mesh` mesh).
  `pause`/`resume`/`speed`/`loop`-toggle deliberately do **not**: they leave the read position alone,
  so the current map is still the map of the frames that produced it.
  **Ordering is the contract, not just the reset.** `_begin_new_timeline()` runs *after*
  `_stop_reader()` and *before* `_spawn()` — the window in which no reader thread exists. Resetting
  after the spawn (which is how the swap handlers used to do it, `await _reset_slam(state)` following
  `await asyncio.to_thread(ctrl.switch_to_*)`) races the new reader, so post-seek frames can land in
  the pre-seek map. That race is what made the same capture at the same seek position reconstruct
  differently depending on where playback had been before. A new timeline-changing operation goes
  through `_begin_new_timeline()`; it must not reset SLAM from the handler
  (`test_swap_handlers_do_not_reset_slam_after_the_swap` pins this). The post-reset window is
  deliberately empty rather than seeded: with no quaternion, `SlamRunner.submit` drops frames exactly
  as it does at every live start-up, so SLAM waits for replay to re-supply orientation instead of
  borrowing the previous timeline's. The CALIB prefix (`build_capture_index` / `_PrefixSource`) is
  unaffected — it is a property of the file, not of the timeline.
