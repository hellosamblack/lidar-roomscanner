...
and let live SLAM consume high-rate frames without forcing the browser to render at
...
4. Make ACK parsing typed rather than a raw three-tuple. Commands 1–8 retain the legacy
   12-byte `<III>` ACK. Commands 9 and 10 return the complete eight-byte applied/readback
   configuration after `cmd` and `result`, so the host can prove what the device applied
   instead of echoing the request. Document exact offsets, units, reserved bits, and
   failure semantics.
...
### Task 7: Rate-aware IMU and barometer behavior
...
1. Write tests proving a 30-second barometer time constant is equivalent at 30 and
   90 Hz (`900` versus `2700` frame samples), including a live rate change that does not
   reset the map or introduce a correction step.
2. Pass applied target FPS with each SLAM submission and update the mapper's
   `baro_tau_frames = round(30.0 * target_fps)` under the worker thread. Carry the same
   scalar through the remote-worker message so local and service backends match.
3. Keep replay truthful: use capture timestamp rate when no live profile target exists.
   Never assume 30 Hz for a 90 Hz recording.
4. Do not merely assign `QUAT_REF_RATE_HZ`: it is currently a derivation/comment constant
   ...
   `ImuFusion` input, recompute only the rate-derived yaw crossover term selected in Task
   1, and leave the existing timestamp-based propagation/tilt gains in seconds. Tests must
   show the effective gain changes while 30 Hz behavior stays numerically unchanged.
5. On a stationary replay resampled to 30 and 90 reference updates, compare orientation
   noise and phase response in seconds, not frames. Reject a change that merely produces
   a different number without preserving the filter's intended time-domain behavior.

**Commit:** `feat(slam): preserve filter time constants across sensor rates`

---

### Task 8: Decouple full-rate SLAM ingest from browser presentation
...
### Task 9: Authoritative web profile state, controls, and bus visualization
...
8. Add a `title` to every new control; extend static tests for tooltip coverage,
...
9. Visually verify at desktop and narrow widths with the headless UI tools. Confirm the
   manual panel does not make the Device card exceed its dock band; use progressive
   disclosure rather than deleting readouts.

**Commit:** `feat(web): add ranging modes and live bus consequences`

---

### Task 10: MCP controls and effect-verified automation
...
4. Test stale broadcasts, timeout after an unchanged state, successful exact readback,
   CDC warning, replay rejection, and two sequential manual changes.

**Commit:** `feat(mcp): expose verified ranging profile control`

---

### Task 11: End-to-end validation, documentation, and landing **[HW]**
...
4. For each acceptance capture record requested/applied config, measured interval
   ...
   firmware TX drops/high-water, link bytes/s, IMU sample rate, transform failures, SLAM
   submissions/processed/replacements, and browser render FPS.
...
6. Drive the real browser UI: all four profile buttons, invalid manual combinations,
   ...
   CDC warning, reconnect/new-tab synchronization, narrow layout, and tooltips.
...
Task 1 contract/baseline
  ...
  -> Task 7 rate-aware filters
  -> Task 8 full-rate SLAM ingest
  -> Task 9 web UI
  -> Task 10 MCP
  -> Task 11 full validation/status sync/retro
```
