...
Completed-work narratives moved out of [`ROADMAP.md`](../ROADMAP.md) when it was split into a
hot current-state doc + this cold archive. **Phases keep their original `Phase N` / `Web Phase N`
/ `6.x` names** so existing references ("ROADMAP Phase 5", "6.D", "6.G") still resolve here.
The forward-looking backlog now lives in `ROADMAP.md` → "Work-item register" with type-prefixed
IDs; this file is the measured record of how each completed piece landed.
...
**Revised 2026-08-04 (stationary ceiling study, `calibration/flatfield/2026-08-04/`):** the FPN is
**mode-family-specific** — Precision-family runs carry a stable **6.6%** high-frequency pattern while
Ambient-family runs carry a different **3.7%** one (map correlation only ~0.34; cross-applying a map
leaves ~6.0–6.4%). A single global `flatfield_path` is therefore **not** the correct end state: enabling
correction needs **mode-aware selection** — at minimum distinguish Precision-like from Ambient, ideally
key by ranging mode × power mode × exposure family — either a mode-keyed map registry or an explicit
operator-selected map, so a Precision map is never silently applied to an Ambient frame. Two engineering
actions fell out, **both now done: (1)** mode-aware `flatfield` selection **SHIPPED 2026-08-04** —
`FlatFieldSet` keyed by ranging mode, `[viewer] flatfield_precision_path`/`flatfield_ambient_path`
(legacy `flatfield_path` = fallback default), the live `TransformStage` pointed at the current mode's
map from the ranging-config ACK, and a web Device-card **Calibrated/Uncalibrated** runtime toggle
(`set_calibrated`, persisted `[viewer] calibrated`); **(2)** DC-D2 — a panned pan per mode family plus a
held-out-distance/second-room pan (the 2026-08-04 static holdout only proved *temporal stability*, not
*scene-independence*; the cross-room captures then proved it: ~2.4% Ambient / ~3.0% Precision) (a static map can retain the ceiling's illumination field,
and the before/after metric shares the builder's 2.5σ low-pass, making part of the reduction definitional).
The nine per-capture candidate maps in that archive are **engineering candidates, not production**; for
interim viewer testing use the 8 ms Precision map for Precision-like modes and the Regular/ULP Ambient
maps for their Ambient modes.
...
>   **⚠ That `AXIS_CONVENTION` verification was empty and the value was wrong (BUG-059, 2026-08-02).** The criterion was |B|, which every one of the 48 signed permutations preserves — including the inverting ones — so it separated nothing. The calibrated field was **anti-parallel** to Earth's (70–72° *above* the horizon; the northern-hemisphere field is that far below), putting magnetic north 180° out: aimed north, the instrument read south, for three weeks. Now `MAG_FIELD_SIGN * MAG_MOUNT_ROTATION` = `-1 * diag(1,-1,-1)`, **det −1 by design**. The axis assignment was right all along — over a 360° sweep magnetic north's bearing must stay constant, and only that assignment and its negation do (sd 6.1° vs 75–79°). Verify with the **dip** (`capture_heading` → `inclination_deg`, must be positive), never with |B|. `mag_cal.json` unaffected: the fit is applied before this matrix.
...
> and PASSED 2026-07-31 (residual 7.3%, all 2268 zones inside [0.5, 1.6]).** ⚠️ **But it does not
> "merely need enabling" (revised 2026-08-04):** the stationary ceiling study
> (`calibration/flatfield/2026-08-04/`) found the FPN is **mode-family-specific** (Precision 6.6% vs
> Ambient 3.7%, cross-map residual ~6%), so a single global map mis-corrects the other family. Enabling
> now needs **mode-aware map selection** + a per-family, held-out-distance pan (**DC-D2**) — see the
> Phase 2.5 follow-up block. Newly **proposed**: sub-phase **6.H** — an audible
...
> *(2026-08-01, **BUG-058**: the fourth instance is the third one's **fix**. `yaw_twist_deg` (world-Z
> swing-twist) is no more a bearing than `quat_yaw_deg` was — with the boresight horizontal, rolling
> about it IS a rotation about a horizontal world axis, so the twist absorbs it degree for degree.
> Owner capture `captures/NorthFacingRoll.bin`, held at one bearing and rolled: reported heading swung
> **154° over 153° of roll**, slope −0.978 against roll and +0.005 against the actual bearing.
> BUG-051's own regression test swept off-axis *bearings* at ONE fixed roll, so it could not see this.
> `YawFusion` was worse and was live: `heading − yaw` differenced a device bearing against a world-Z
> twist, so the device term never cancelled and the fused quat came out **mirrored**, its boresight at
> −bearing — invisible to a suite whose every assertion was a magnitude. Fixed by dropping scalar yaw
> outright: `magnetic_north_bearing_deg` (how far the quat's own +X datum sits from north; reads no
> body axis, so no singularity) IS the fusion's correction, and `absolute_heading` is the boresight's
> bearing minus north's, both in one frame, so the datum cancels and roll cannot move it — `None`
> within 10° of vertical, where no bearing exists. Live-verified on that capture: slope **+0.022**.
> **The rule loses its escape hatch: no scalar "yaw" taken off an attitude is the bearing of an axis
> — ask for the bearing of the axis you mean.**)*
> UI gained raw
