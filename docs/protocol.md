...
| 7 | RAW_3DMD | opaque vendor raw frame from the VL53L9CX (input to vl53l9-transform-c). At binning 2: `payload_len` = 14842. Header `width`/`height` carry the logical zone grid (54×42); `payload_len` is authoritative for size. `seq`/`t_us` as for DEPTH frames. The **last 100 bytes are a `vl53l9_meta_t` block** (per-frame die temp, exposure via `nb_shot_step`, error status, dss/binning, reference channels) — decoded host-side by `roomscan.protocol.decode_tof_meta()` (2026-08-06); the firmware itself reads only `frame_counter` from it. | live (Phase 2) |
